#!/usr/bin/env python3
"""
generate_lesson_plan.py - 专业级详案Word文档生成器 v4.0
...
def _add_para(doc, text, bold=False, size=12, indent=0, align=None, color=None, font_name='宋体',
    ...
    """添加带格式的段落"""
    text = _safe_para_text(text)  # 清理占位符
    p = doc.add_paragraph()
    if align:
        p.alignment = align
    if indent:
...
def _validate_no_placeholders(data: dict) -> list:
    """检查数据中是否包含 ___ 占位符，返回警告列表"""
    warnings = []
    # 匹配精确的 ___ 占位符（3个连续下划线且两侧非下划线/字母数字）
    # 排除填空线（如 __________）
    PLACEHOLDER_RE = re.compile(r'(?<![_\w])_{3}(?![_\w])')

    def _check_value(path, val):
        if isinstance(val, str):
            # 只检查关键教学内容字段
            critical_paths = ['textbook_analysis', 'student_analysis', 'teacher_activity',
                            'student_activity', 'design_intent', 'preset_generation']
            is_critical = any(cp in path for cp in critical_paths)
            if is_critical and PLACEHOLDER_RE.search(val):
                warnings.append(f"⚠ {path}: 包含占位符 → '{val[:80]}...'")
        elif isinstance(val, dict):
            for k, v in val.items():
                _check_value(f"{path}.{k}", v)
        elif isinstance(val, list):
            for i, v in enumerate(val):
                _check_value(f"{path}[{i}]", v)

    _check_value("root", data)
    return warnings


def _safe_para_text(text: str) -> str:
    """清理文本中的占位符，替换为有意义的提示"""
    if not text:
        return text
    # 仅替换 "___" 这种3连下划线占位符（保留填空线 ________）
    # 占位符特征：前后是汉字/标点/空白，长度恰好3个下划线
    text = re.sub(r'(?<![_\w])_{3}(?![_\w])', '……', text)
    return text
...
def create_lesson_plan(data: dict, output_path: str):
    """生成专业级详案Word文档"""
    # 验证无占位符
    warnings = _validate_no_placeholders(data)
    if warnings:
        print("⚠️  数据质量警告：", file=sys.stderr)
        for w in warnings:
            print(f"  {w}", file=sys.stderr)
        print("  教案仍会生成，但部分内容可能不完整。", file=sys.stderr)
    ...
    info_data = [
        ...
        ('课    题', topic),
        ('课    时', f'第{lesson}课时' if lesson else '待定'),
        ('教材页码', f'第{textbook_page}页' if textbook_page else '待定'),
    ]
    ...
    if textbook_analysis:
        ...
        sections_data = [
            ('1. 本课在教材中的地位',
             '本课是……年级……册第……单元的第……课。本单元以"……"为主题，'
             '共编排了……篇课文/……个教学内容。本课在单元中处于……位置，'
             '前承……（前一课/前一单元的核心知识），为本课学习奠定了……基础；'
             '后启……（后续课/后续单元的知识），本课的学习将为……做好铺垫。\n\n'
             '【人教版教师教学用书要点】本课教材编写的主要意图在于……，'
             '体现了课程标准中"……"的要求。\n\n'
             '【昌明师友·教学设计要点】本课教学应重点关注……，'
             '建议采用……的方法突破重难点。'),
            ('2. 教材内容深度分析',
             '本课教材主要包含以下核心内容：\n'
             '（1）……——这是本课的基础知识/基本技能\n'
    ...
    if student_analysis:
        ...
        items = [
            ('1. 知识基础', '学生已经学习了……，掌握了……，具备了……的基础知识和基本技能。'
             '但对……的理解还不够深入，需要在本课中进一步巩固和提升。'),
            ('2. 认知特点', '……年级学生正处于……阶段（具体运算阶段/前运算阶段等），'
             '思维方式以……为主（形象思维为主，逐步向抽象思维过渡），'
             '对……较为敏感（直观、生动、有趣的内容），注意力持续时间约……分钟。'),
            ('3. 学习习惯', '学生已初步形成……的学习习惯（如：认真听讲、积极发言、合作学习等），'
             '但在……方面仍需加强（如：独立思考、自主探究、规范表达等）。'
             '班级整体学习氛围……，学优生约……人，学困生约……人。'),
            ('4. 困难预判', '基于对学生的了解，预判本课学习中学生可能遇到以下困难：\n'
             '   ①……（认知层面的困难）\n'
             '   ②……（技能层面的困难）\n'
             '   ③……（情感态度层面的困难）\n\n'
             '   针对以上困难，将在教学过程中通过……予以化解。'),
        ]
    ...
    if objectives:
        ...
        dims_template = [
            ('（一）知识与技能目标', [
                '1. 认识并会写本课生字词……个，理解由生字组成的词语和重点句子的意思。',
                '2. 正确、流利、有感情地朗读课文，背诵……段落。',
                '3. 理解课文主要内容，能用自己的话说说……。',
            ]),
            ('（二）过程与方法目标', [
                '1. 通过朗读品味语言，体会文章的表达方式（如……）。',
        ...
        default_steps = [
            {
                ...
                'teacher': (
                    '① 教师话术："同学们，老师今天给大家带来了一样神奇的东西（出示/展示……）。'
                    '你们知道这是什么吗？"\n'
                    '② 追问："有谁见过/听说过……？能给大家说说吗？"\n'
                    '③ 引导："看来同学们对……很感兴趣。那你们想不想知道……？"\n'
                    '④ 过渡语："好，今天我们就一起来学习——《……》。"（板书课题）\n'
                    '⑤ 板书动作：在黑板正中书写课题，字迹工整，大小适中。'
                ),
                'student': (
                    '① 预设A：学生积极举手回答 → 教师："你观察得真仔细！还有谁想补充？"\n'
                    '② 预设B：学生沉默 → 教师提示："想想看，你在……的时候有没有见过……？"\n'
                    '③ 预设C：学生回答偏离 → 教师引导："你说得很有趣，不过我们现在先来关注……。"'
                ),
                'intent': '从学生已有的生活经验出发，创设真实情境，激发学习兴趣和探究欲望。'
                         '符合课标"联系生活实际，创设真实情境"的教学建议。',
                'preset': (
                    '预设A（顺利）：学生能说出……→ 教师跟进："说得很好！那你们知道它是怎么来的吗？"\n'
                    '预设B（部分）：学生只能说出表面 → 教师追问："除了……，你还发现了什么？"\n'
                    '预设C（困难）：学生完全不了解 → 教师降低难度："没关系，我们先来看看……。"'
                ),
                'transition': '刚才同学们都观察得很仔细，接下来，让我们打开课本，一起走进……的世界。',
            ...
            },
            {
                ...
                'teacher': (
                    '① 任务布置："现在请大家打开课本待定，自由朗读课文，注意读准字音，读通句子。"\n'
                    '② 巡视指导：关注学生的朗读情况，个别指导读音困难的学生。\n'
                    '③ 检查反馈："好，我们来看看这些生字词。（出示PPT）谁来读一读？"\n'
                    '④ 精读引导："课文第……段写了……，请同学们仔细读一读，'
                    '找一找哪些词句最能体现……？用笔画出来。"\n'
                    '⑤ 点拨提升："同学们找得很好。我们来看……这个词/句，'
                    '它用了……的方法，写出了……。"\n'
                    '⑥ 板书核心知识：在黑板上结构化呈现本课知识要点。'
                ),
                'student': (
                    '① 学生自由朗读课文，标注生字词和重点句子。\n'
                    '② 预设A：学生准确找出关键词句 → 教师追问"为什么这个词/句能体现……？"\n'
                    '③ 预设B：学生找得不够准确 → 教师提示"再读一遍第……句，想想……"\n'
                ...
                'preset': (
                    '预设A（读音问题）：学生读错……字 → 教师："这个字的读音是……，注意……的发音。"\n'
                    '预设B（理解偏差）：学生理解为…… → 教师引导："你的理解有道理，但我们再看看上下文，'
                    '作者想表达的是……。"\n'
                    '预设C（深度提问）：学生问"……？" → 教师赞赏："这个问题提得很好！'
                    '谁能帮他解答？"或教师直接解答。'
                ),
                'transition': '通过刚才的学习，我们了解了……。接下来，请同学们小组合作，讨论……。',
            ...
            },
            {
                ...
                'preset': (
                    ...
                    '哪种更合理？为什么？"\n'
                    '预设C（讨论不到位）：教师提供思考支架"我们来从……角度想想看。"'
                ),
                'transition': '同学们讨论得很热烈，汇报也很精彩！通过合作学习，我们发现……。'
            ...
            },
            {
                ...
                'teacher': (
                    ...
                    '② 巡视收集典型错误，拍照或记录。\n'
                    '③ 集体讲评："我们一起来看第……题，大部分同学都做对了。'
                    '第……题有同学出错了，我们来看看错在哪里。"\n'
                    '④ 错误分析："这位同学的答案是……，你能说说你的想法吗？'
                    '（倾听后）原来你是这样想的。正确的方法应该是……。"\n'
                ...
                'transition': '通过练习，大家对……掌握得越来越好了。接下来，让我们一起回顾一下今天学到了什么。',
            ...
            },
            {
                ...
                'teacher': (
                    '① 引导回顾："同学们，这节课我们学习了《……》。谁能说说，你学到了什么？"\n'
                    '② 师生共同梳理：（指着板书）"我们一起回顾一下：'
                    '首先我们……，然后……，最后……。"\n'
                    '③ 升华提升："通过今天的学习，我们不仅知道了……，'
                    '更重要的是学会了……的方法。"\n'
                    '④ 情感引导："……（联系课文情感/价值观升华）。"'
                ),
                'student': (
            ...
            },
            {
                ...
                'teacher': (
                    ...
                    '② 基础作业（必做）："第一项，……。这是每个人都要完成的。"\n'
                    '③ 提高作业（选做）："第二项，……。有兴趣的同学可以挑战一下。"\n'
                    '④ 拓展作业（挑战）："第三项，……。学有余力的同学可以试试。"\n'
